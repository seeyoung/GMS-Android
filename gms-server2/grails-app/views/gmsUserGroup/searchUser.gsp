...
<html>
	<head>
		...
		<title><g:message code="default.add.label" args="[entityName]" /> [ <g:message code="gmsUserGroup.label" default="GmsUserGroup" />: ${gmsUserGroupInstance.name} ]</title>
		...
		<r:script disposition="head">
			...
			function addUser(userId){
				$.ajax({
						url:"${createLink(action:'addUser',id:params.id)}",
						data: {userId:userId},
	...
	<body >
		<div id="list-gmsUser" class="content scaffold-list" role="main">
			...
			<g:else>
				<g:message code="default.add.dblClick.message" args="[entityName]" default="Double Click an user to add." /><br/>
			</g:else>
			</div>
			
			<g:form name="searchForm" action="searchUser" method="PUT" >
				<g:hiddenField name="id" value="${gmsUserGroupInstance.id}" />
			...
			<thead>
				...
				<g:each in="${gmsUserInstanceList}" status="i" var="gmsUserInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}" onDblClick="addUser('${fieldValue(bean: gmsUserInstance, field: "id")}')">
			...
			<div class="pagination" style="height:30px">
				...
				<fieldset class="buttons" style="text-align:center;">
					<a class="close" href="javascript:opener.refresh();window.close();"><g:message code="default.button.close.label" default="Close" /></a>
				</fieldset>
		</div>
	</body>
</html>
